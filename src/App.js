...
function App() {
  ...
  const removeVillager = (villager) => {
    const villagerName = villager.toLowerCase();

    if (villagers.includes(villagerName)) {
      const filteredVillagers = villagers.filter(
        (villager) => villager !== villagerName
      );
    ...
    const filteredVillagerData = villagerData.filter(
      (villager) => villager.name.toLowerCase() === villager
    );

    console.log(filteredVillagerData);

    // setVillagerData(filteredVillagerData);
  };
